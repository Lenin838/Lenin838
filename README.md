<div align="center">

<img width="100%" src="profile/github_banner.png" alt="Lenin Joseph — MERN Stack Developer" />

<br/>

<picture>
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=14&pause=2500&color=13B9FD&center=true&vCenter=true&width=650&lines=MERN+Stack+Developer+%C2%B7+Ernakulam%2C+Kerala;Building+Scalable+Web+Applications;Solving+Real-World+Problems+with+Code" alt="MERN Stack Developer · Ernakulam, Kerala" />
</picture>

<br/><br/>

```text
L  E  N  I  N     J  O  S  E  P  H
```

<sub><sup>MERN STACK DEVELOPER  ·  PROBLEM SOLVER  ·  FULL-STACK BUILDER</sup></sub>

<br/><br/>

[![GitHub](https://img.shields.io/badge/GitHub-0d0d14?style=flat\&logo=github\&logoColor=ffffff)](https://github.com/Lenin838) 
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0d0d14?style=flat\&logo=linkedin\&logoColor=0A66C2)](https://www.linkedin.com/in/lenin-joseph-746083340/) 
[![Email](https://img.shields.io/badge/Email-0d0d14?style=flat\&logo=gmail\&logoColor=EA4335)](mailto:leninjoseph32@gmail.com)

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=Lenin838&color=13B9FD&style=flat&label=PROFILE+VIEWS" alt="Profile Views" />

</div>

<br/>

---

<br/>

## 🧠 About Me

```javascript
const LeninJoseph = {
  role: "MERN Stack Developer",
  location: "Ernakulam, Kerala, India",

  focus: [
    "Scalable web applications",
    "Backend architecture",
    "RESTful APIs",
    "Real-time systems",
    "Database design",
    "Problem solving"
  ],

  currentlyBuilding: [
    "Production-oriented full-stack applications",
    "Real-time backend systems",
    "Clean and maintainable APIs"
  ],

  philosophy:
    "Build simple, scalable solutions to real-world problems."
};
```

I’m a **MERN Stack Developer** focused on building practical, scalable web applications and solving real-world engineering problems.

My work spans **frontend development, backend architecture, REST APIs, MongoDB, authentication, payment systems, real-time communication, and application design**.

I enjoy taking an idea from **architecture → implementation → testing → deployment** and continuously improving the way I build software.

<br/>

---

<div align="center">
<sub>⚡ ENGINEERING FOCUS</sub>
</div>

<br/>

<div align="center">

`Full-Stack Development`   `Backend Engineering`   `REST APIs`
`Database Design`   `Real-Time Systems`   `Authentication`
`System Design`   `Problem Solving`   `Clean Architecture`

</div>

<br/>

---

<br/>

<div align="center">
<sub>🛠️ TECH STACK</sub>
</div>

<br/>

<div align="center">

### Frontend

![HTML5](https://img.shields.io/badge/HTML5-0d0d14?style=for-the-badge\&logo=html5\&logoColor=E34F26)
![CSS3](https://img.shields.io/badge/CSS3-0d0d14?style=for-the-badge\&logo=css3\&logoColor=1572B6)
![JavaScript](https://img.shields.io/badge/JavaScript-0d0d14?style=for-the-badge\&logo=javascript\&logoColor=F7DF1E)
![React](https://img.shields.io/badge/React-0d0d14?style=for-the-badge\&logo=react\&logoColor=61DAFB)
![Bootstrap](https://img.shields.io/badge/Bootstrap-0d0d14?style=for-the-badge\&logo=bootstrap\&logoColor=7952B3)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-0d0d14?style=for-the-badge\&logo=tailwindcss\&logoColor=06B6D4)
![EJS](https://img.shields.io/badge/EJS-0d0d14?style=for-the-badge\&logo=ejs\&logoColor=A91E50)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-0d0d14?style=for-the-badge\&logo=node.js\&logoColor=339933)
![Express.js](https://img.shields.io/badge/Express.js-0d0d14?style=for-the-badge\&logo=express\&logoColor=ffffff)
![REST API](https://img.shields.io/badge/REST_APIs-0d0d14?style=for-the-badge\&logo=fastapi\&logoColor=009688)
![Socket.IO](https://img.shields.io/badge/Socket.IO-0d0d14?style=for-the-badge\&logo=socket.io\&logoColor=ffffff)

### Database

![MongoDB](https://img.shields.io/badge/MongoDB-0d0d14?style=for-the-badge\&logo=mongodb\&logoColor=47A248)
![Mongoose](https://img.shields.io/badge/Mongoose-0d0d14?style=for-the-badge\&logo=mongoose\&logoColor=880000)

### Authentication & Integrations

![Passport.js](https://img.shields.io/badge/Passport.js-0d0d14?style=for-the-badge\&logo=passport\&logoColor=34E27A)
![Google OAuth](https://img.shields.io/badge/Google_OAuth-0d0d14?style=for-the-badge\&logo=google\&logoColor=4285F4)
![Razorpay](https://img.shields.io/badge/Razorpay-0d0d14?style=for-the-badge\&logo=razorpay\&logoColor=3395FF)
![Nodemailer](https://img.shields.io/badge/Nodemailer-0d0d14?style=for-the-badge\&logo=minutemailer\&logoColor=ffffff)

### Architecture & Engineering

![MVC](https://img.shields.io/badge/MVC_Architecture-0d0d14?style=for-the-badge\&logoColor=4ade80)
![Repository Pattern](https://img.shields.io/badge/Repository_Pattern-0d0d14?style=for-the-badge\&logoColor=4ade80)
![Dependency Injection](https://img.shields.io/badge/Dependency_Injection-0d0d14?style=for-the-badge\&logoColor=4ade80)
![WebSockets](https://img.shields.io/badge/WebSockets-0d0d14?style=for-the-badge\&logo=websocket\&logoColor=13B9FD)

### Tools

![Git](https://img.shields.io/badge/Git-0d0d14?style=for-the-badge\&logo=git\&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-0d0d14?style=for-the-badge\&logo=github\&logoColor=ffffff)
![Postman](https://img.shields.io/badge/Postman-0d0d14?style=for-the-badge\&logo=postman\&logoColor=FF6C37)
![VS Code](https://img.shields.io/badge/VS_Code-0d0d14?style=for-the-badge\&logo=visualstudiocode\&logoColor=007ACC)

</div>

<br/>

---

<br/>

<div align="center">
<sub>🚀 FEATURED PROJECTS</sub>
</div>

<br/>

### 👜 Ryzo Bags

**Full-Stack E-Commerce Platform**

A complete e-commerce application built with **Node.js, Express.js, MongoDB and EJS**, following MVC architecture.

**Highlights**

* 🔐 Authentication & authorization
* 📧 Email OTP verification
* 🔑 Google OAuth
* 🛒 Shopping cart & wishlist
* 💳 Razorpay payment integration
* 🎟️ Coupon system
* 💰 Wallet & Cash on Delivery
* 📦 Inventory & order management
* 📊 Admin analytics dashboard
* 📄 PDF & Excel sales reports
* 🔒 Rate limiting & security middleware

**Architecture:** MVC · RESTful Routing · Middleware-based Authorization · MongoDB

[![View Project](https://img.shields.io/badge/View_Project-13B9FD?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Lenin838/Ryzobags)

---

### 🔔 Notification Router

**Fault-Tolerant Real-Time Notification System**

A production-oriented notification routing system designed around **priority queues, automatic provider failover and real-time monitoring**.

**Highlights**

* ⚡ Priority-based notification processing
* 🔄 Automatic provider failover
* 📬 SMS → Email fallback strategy
* 🌐 RESTful API
* 🔌 WebSocket / Socket.IO communication
* 📊 Live monitoring dashboard
* 🧱 Repository Pattern
* 💉 Dependency Injection
* 🏗️ Service-layer architecture
* 🗄️ MongoDB persistence
* 🔍 Delivery pipeline logging

**Architecture:** React → Express → Notification Service → Priority Queue → Providers → MongoDB → Socket.IO

[![View Project](https://img.shields.io/badge/View_Project-13B9FD?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Lenin838/notification-router)

---

<br/>

---

<div align="center">
<sub>📊 GITHUB ACTIVITY</sub>
</div>

<br/>

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Lenin838&show_icons=true&hide_border=true&bg_color=0d0d14&title_color=13B9FD&icon_color=54C5F8&text_color=ccccdd&rank_icon=github" alt="Lenin Joseph GitHub Stats" />

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Lenin838&layout=compact&hide_border=true&bg_color=0d0d14&title_color=13B9FD&text_color=ccccdd&langs_count=8" alt="Top Languages" />

<br/><br/>

<img src="https://streak-stats.demolab.com?user=Lenin838&hide_border=true&background=0d0d14&ring=13B9FD&fire=13B9FD&currStreakLabel=13B9FD&sideLabels=ccccdd&currStreakNum=ccccdd&sideNums=ccccdd&dates=888899" alt="GitHub Streak" />

</div>

<br/>

---

<br/>

<div align="center">
<sub>🧩 PROBLEM SOLVING</sub>
</div>

<br/>

<div align="center">

I regularly practice **Data Structures & Algorithms** to strengthen problem-solving skills,
algorithmic thinking, and coding fundamentals.

<br/><br/>

[![LeetCode](https://img.shields.io/badge/LeetCode-Profile-0d0d14?style=for-the-badge\&logo=leetcode\&logoColor=FFA116)](https://leetcode.com/)

</div>

<br/>

---

<br/>

## 🎯 What I'm Working On

```text
┌──────────────────────────────────────────────────────┐
│                                                      │
│  ▸ Building scalable MERN applications               │
│                                                      │
│  ▸ Improving backend architecture & system design     │
│                                                      │
│  ▸ Exploring real-time distributed systems            │
│                                                      │
│  ▸ Strengthening Data Structures & Algorithms         │
│                                                      │
│  ▸ Writing cleaner, maintainable production code      │
│                                                      │
└──────────────────────────────────────────────────────┘
```

<br/>

---

<br/>

<div align="center">
<sub>🤝 LET'S CONNECT</sub>

<br/><br/>

I'm interested in collaborating on interesting projects,
learning from other developers, and building software that solves meaningful problems.

<br/><br/>

[![GitHub](https://img.shields.io/badge/GitHub-Lenin838-0d0d14?style=for-the-badge\&logo=github\&logoColor=white)](https://github.com/Lenin838)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Lenin_Joseph-0d0d14?style=for-the-badge\&logo=linkedin\&logoColor=0A66C2)](https://www.linkedin.com/in/lenin-joseph-746083340/)
[![Email](https://img.shields.io/badge/Email-Contact_Me-0d0d14?style=for-the-badge\&logo=gmail\&logoColor=EA4335)](mailto:leninjoseph32@gmail.com)

<br/><br/>

<sub>Ernakulam, Kerala, India  ·  MERN Stack Developer  ·  Open to Collaboration</sub>

<br/><br/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=400&size=12&pause=9999&color=556070&center=true&vCenter=true&width=450&lines=Build+%C2%B7+Learn+%C2%B7+Solve+%C2%B7+Improve" alt="Build · Learn · Solve · Improve" />

</div>

<br/>

---

<div align="center">

⭐ **If you find something useful in my repositories, consider giving it a star.**

</div>

<!-- last-updated: 2026-09-07 -->
